...
// ─── DIZIONARIO ESERCIZI → VIDEO ID REALI ─────────────────
const EXERCISE_YT = {
  // Petto
  ...
  't-bar row':             'j3Ss2MxIHh4',
  // Spalle
  'overhead press':        '2yjwXTZbDtQ',
  'shoulder press':        '2yjwXTZbDtQ',
  'lateral raise':         'XPPfnSEATJA',
  'front raise':           'sOT-eFMOGiI',
  'face pull':             'eIq5CB9JfKE',
  'arnold press':          '6Z15_WdXmVw',
  'rear delt fly':         'xpS1YOlCRBE',
  // Bicipiti
  ...
  'skull crusher':         'd_KpYHJwWVg',
  'overhead tricep':       'YbX7Wd8jQ-Q',
  'close grip bench':      'nEF0bv2FW54',
  'tricep dip':            'ybjSaU0mS3I',
  // Gambe
  'squat':                 'ultWZbUMPL8',
  'leg press':             'IZxyjW7SKSA',
  'leg extension':         'YyvSfVjQeL0',
  'leg curl':              'ELOCsoDSmrg',
  'romanian deadlift':     'JCXUYuzwNrM', // Già sopra, ma raddoppio ok
  'lunges':                'QOVaHwm-Q6U',
  'calf raise':            'gwLzBJYoWlA',
  'hack squat':            'EdtPAJ0WoMo',
  'bulgarian split squat': 'cv9GhMbpOd8',
  'hip thrust':            'SEdqd9BNs5E',
...
async function validateYtId(id) {
  return new Promise(resolve => {
    const img = new Image();
    img.onload  = () => resolve(img.naturalWidth >= 320);
...
async function searchYouTubeId(query) {
  const tryInstance = async (base) => {
    ...
    if (!id || id.length !== 11) throw new Error();
    return id;
  };
  try {
    return await Promise.any(PIPED_INSTANCES.map(b => tryInstance(b)));
  } catch {}
  return null;
}
...
async function resolveExerciseMedia(name) {
  ...
  // 3. Dizionario hardcoded
  const lc = englishName.toLowerCase();
  const dictKey = Object.keys(EXERCISE_YT).find(k => lc.includes(k) || k.includes(lc.split(' ')[0]));
  if (dictKey) {
    const candidate = EXERCISE_YT[dictKey];
    const valid = await validateYtId(candidate);
    if (valid) ytId = candidate;
  }

  // 4. Piped (se non trovato nel dizionario)
  if (!ytId) {
    ytId = await searchYouTubeId(`${englishName} exercise tutorial proper form`);
  }

  // 5. Groq suggerisce ID da canali noti + validazione
  if (!ytId && key) {
    try {
      const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `What is the exact 11-character YouTube video ID of the most famous tutorial for "${englishName}" exercise? Choose from AthleanX, Jeff Nippard, Scott Herman or similar. Return ONLY the 11-character ID.`
          }],
          temperature: 0, max_tokens: 15
        })
      });
      const d = await res.json();
      const candidate = d?.choices?.[0]?.message?.content?.trim().replace(/[^a-zA-Z0-9_\-]/g, '');
    ...
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
        ...
        <div style={{
          ...
          {!media
            ? <span className="pls" style={{ fontSize: 28 }}>🏋️</span>
            : media.iconUrl
              ? <img
                  src={media.iconUrl}
                  className="ex-icon"
        ...
        {/* Bottone — appare SOLO se abbiamo un ID valido */}
        {!media
          ...
          : media.ytId
            ? <button
                className="btn btn-s"
                style={{
                  fontSize: 12, padding: '9px 14px', gap: 6,
      ...
      {/* Embed 16:9 — solo se abbiamo l'ID */}
